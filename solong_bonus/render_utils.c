...
/*   Updated: 2024/06/11 16:44:33 by ehammoud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw(t_map g, t_tex t, int i, int j)
{
	...
	if ((g.map[i][j] == 'C' || g.map[i][j] == 'G') && g.frame < 18 * g.speed)
		mlx_put_image_to_window(g.mlx, g.mw, \
			t.w.cd[g.frame / g.speed], j * 64, i * 64);
	else if (g.map[i][j] == 'C' || g.map[i][j] == 'G')
		mlx_put_image_to_window(g.mlx, g.mw, \
			t.w.cd[35 - (g.frame / g.speed)], j * 64, i * 64);
	if (g.map[i][j] == 'X' || g.map[i][j] == 'Q' || g.map[i][j] == 'G')
